...
Abrir opções em `http://localhost:8080`.


Teste:

curl -X 'POST' \
  'http://localhost:8080/entregas/1/relatorio' \
  -H 'accept: */*' \
...
no producer:
curl -X 'GET' 'http://localhost:8080/entregas/1/relatorio/2'

no processor:
curl -X 'GET' 'http://localhost:8081/entregas/1/relatorio/2'
...
(Rodar em cada um dos contextos (dentro do producer e dentro do processos)
